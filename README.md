# 👋 Pedro Leonardo

## Desenvolvedor Back-end Java | Spring Boot | APIs REST

Sou **Desenvolvedor Back-end em formação**, com foco em **Java e Spring Boot**, atuando na construção de **APIs REST escaláveis**, regras de negócio bem definidas e integração com bancos de dados relacionais.

Tenho perfil analítico, foco em boas práticas, código limpo e organização de camadas. Busco minha **oportunidade desenvolvedor júnior**, onde eu possa aprender com times experientes e contribuir com soluções bem estruturadas.

---

## 🧠 Perfil Técnico

* Desenvolvimento de APIs REST seguindo padrões RESTful
* Implementação de regras de negócio e validações
* Tratamento global de erros e respostas HTTP adequadas
* Integração com banco de dados relacional
* Organização em camadas (Controller, Service, Repository)
* Versionamento de código com Git

---

## 🛠️ Tecnologias

### Back-end

* Java
* Spring Boot
* Spring Web
* Spring Data JPA
* Hibernate

### Banco de Dados

* PostgreSQL
* MySQL (conceitos)

### Ferramentas

* Git & GitHub
* Swagger / OpenAPI
* IntelliJ IDEA
* Postman

---

## 📂 Projetos

### 🔹 API de Gerenciamento de Produtos e Categorias

Projeto back-end desenvolvido simulando um **sistema corporativo de catálogo de produtos**, com foco em organização de código, regras de negócio e boas práticas.

**Responsabilidades técnicas:**

* Desenvolvimento de API REST com Spring Boot
* Implementação de CRUD completo para produtos e categorias
* Validações de entrada de dados
* Tratamento de exceções personalizadas
* Uso de DTOs para entrada e saída de dados
* Documentação da API com Swagger
* Persistência de dados com JPA/Hibernate e PostgreSQL

---

### 🔹 Sistema de Gerenciamento de Veículos

Aplicação desenvolvida em Java com foco em **Programação Orientada a Objetos**, simulando um sistema interno para controle de veículos.

**Responsabilidades técnicas:**

* Aplicação de conceitos de POO (encapsulamento, herança e polimorfismo)
* Organização do domínio da aplicação
* Uso de listas dinâmicas para armazenamento de dados
* Estruturação do código visando manutenção e evolução

---

## 📈 Em desenvolvimento

* Java avançado
* Spring Boot (boas práticas e arquitetura)
* Testes automatizados (conceitos)
* Versionamento profissional com Git

---

## 🎯 Objetivo Profissional

Atuar como **Desenvolvedor Back-end Júnior**, contribuindo no desenvolvimento de APIs, regras de negócio e sistemas escaláveis, ao mesmo tempo em que evoluo tecnicamente e profissionalmente.

---

## 📬 Contato

* GitHub: *(https://github.com/Dev-lleonardo)*
* LinkedIn: *(https://www.linkedin.com/in/pedrolleonardo/)*
* E-mail: *(contatolleonardo.dev@gmail.com)*

---

# 🗺️ Roadmap de Projetos Back-end

### 🔹 Nível 1 – Base (Atual)

* API REST com Spring Boot
* CRUD completo
* Integração com banco de dados

### 🔹 Nível 2 – Intermediário

* Autenticação com Spring Security
* JWT
* Perfis de usuário
* Paginação e filtros avançados

### 🔹 Nível 3 – Avançado

* Testes automatizados (JUnit e Mockito)
* Dockerização da aplicação
* Logs e monitoramento
* Deploy em cloud (Railway, Render ou AWS)

### 🔹 Nível 4 – Profissional

* Arquitetura em camadas bem definidas
* Boas práticas de Clean Code
* Versionamento profissional
* Simulação de ambiente corporativo
